    ...
    class HomeService
    {
     //   Define as singLeton;
        public List<home> afficher()
        {
            ...
            sqlConn.ConnectionString = "SERVER=localhost; DATABASE=smarthome; UID=root; PASSWORD=";
        ...
        public bool killSwitch2()
        {
            MySqlCommand cmd = Connection.getMySqlCommand();
            cmd.CommandText = "delete from zone";
            cmd.ExecuteNonQuery();
            return true;
        }

        public bool LogUser(Users u)
        {

            MySqlDataAdapter MyDA = new MySqlDataAdapter();
            string sqlSelectAll = "SELECT * from users where name='" + u.Name + "'AND password='" + u.Password + "'";
        ...
        public Users AfficherUser(Users u)
        {
            Users us=null;
            MySqlConnection sqlConn = new MySqlConnection();
            MySqlCommand sqlCmd = new MySqlCommand();
            string sSql = "SELECT * from users where name='" + u.Name + "'AND password='" + u.Password + "'";
            sqlCmd.CommandText = sSql;
            sqlCmd.CommandType = CommandType.Text;
            sqlConn.Open();
            sqlCmd.Connection = sqlConn;
            MySqlDataReader reader = sqlCmd.ExecuteReader();
            if (reader.Read())
         us  = new Users(reader["name"].ToString(), reader["password"].ToString(), reader["role"].ToString() );
        ...
        {
            MySqlCommand cmd = Connection.getMySqlCommand();
            cmd.CommandText = "INSERT INTO zone(name,height,width) VALUES(@name,@height,@width)";
            cmd.Parameters.AddWithValue("@name", z.Name);
            cmd.Parameters.AddWithValue("@height", z.Height);
            cmd.Parameters.AddWithValue("@width", z.Width);
            cmd.ExecuteNonQuery();
            return true;
        }
    }
